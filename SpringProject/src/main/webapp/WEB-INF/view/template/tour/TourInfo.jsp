						...
						if(<%=memLogin%> == null && <%=corpLogin%> == null){
							...
							$.ajax({
								...
								success : function(data) {
									$('.commentTable tr').remove();
									$.each(data,function(key,value) {
										$('.commentTable').append('<tr><td class="memIdCell"><div class="memId">'+ value.memberId+ '&nbsp;&nbsp;</div></td><td><div class="semo"></div></td><td><div class="contNdate">'+ value.content + '<br><label class="commentDate">' + value.commentDate+ '</label></div></td></tr><tr><td><br></td></tr>');
										});
								},
								error : function(request,status,error) {
									console.log("code:"	+ request.status+ "\n"+ "error:"+ error);
